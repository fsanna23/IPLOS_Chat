...
* **`dbService.js`**: Manages the connection to a PostgreSQL database using connection pooling. It provides the `saveToDB` function used by `chatService.js` to persist the extracted JSON answers.
* **`excelService.js`**: Contains logic for exporting data to an `.xlsx` file using `exceljs`. *(Note: The main execution flow primarily relies on `dbService.js` for data persistence, but this service provides an alternative output format).*
...
2. Create a `.env` file in the root directory and add the following configuration variables:
   ...
   # If using Supabase, get your Connection String (URI) from Project Settings > Database
   # Make sure to use the connection pooler port (usually 6543)
...
2. **Deploy to Cloud Run:**
   ...
   ```
   *(Note: For production, consider using Google Cloud Secret Manager to securely handle the `GEMINI_API_KEY` and `DATABASE_URL` rather than plain text environment variables).*
